...
sampleList = [];
for i = 1:numSamples
    angle1 = 2*pi*rand(1) - pi;
    angle2 = 2*pi*rand(1) - pi;
...
fprintf('Found %d free configuration space points out of %d samples\n', size(freeCSpacePoints,1), size(sampleList,1));

%% Compute adjacency look-up table suitable for doing BFS

%connectRadius = (3.2*pi)/(numSamples-1);
connectRadius = .3;
...
for i = 1:3
    ...
    % Plot the free configuration connections
%     for j = 1:size(freeCSpacePoints,1)
%         
